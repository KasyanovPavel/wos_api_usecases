...
and visualise policy documents analytics and relations between
scholarly articles and policy documents. The application retrieves data
using Web of Science Expanded API.
...
def start_menu_wos() -> str:
    """Manage Flask interface actions and rendering for Societal
    ...
    if request.method == 'POST' and 'button' in request.form.keys():
        visualizations = {
            ...
            'top_cited_authors': plots_list[1],
            'top_citing_authors': plots_list[2],
            'top_citing_sources': plots_list[3],
            'top_citing_source_countries': plots_list[4]
        }
...
def render_trends_query(search_query: str) -> str:
    ...
    response_2 = validate_search_query_pci(search_query)
    if response_1[0] == response_2[0] == 200:
        return render_template(
            'trends.html',
            message_1=f'Web of Science documents found: {response_1[1]}',
            message_2=f'Policy Citation Index documents found: '
                      f'{response_2[1]}',
            search_query=search_query[3:]
        )
    if response_1[0] == 200:
        return render_template(
            'trends.html',
            message_1=f'Web of Science documents found: {response_1[1]}',
            error_message_2=f'Policy Citation Index request status:'
